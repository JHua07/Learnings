...
public:
    ...
    {
        N = sigv.N;                       // 体素内点云的数量
        v = stat.R * sigv.v + N * stat.p; // 体素内点云的质心
        Eigen::Matrix3d rp = stat.R * sigv.v * stat.p.transpose();
        P = stat.R * sigv.P * stat.R.transpose() + rp + rp.transpose() + N * stat.p * stat.p.transpose(); // 体素内点云的协方差矩阵
    }
};
...
public:
    ...
    void push_voxel(const vector<VOX_FACTOR> *sig_orig, const vector<PLV(3)> *vec_orig)
    {
        ...
        {
            // 从这里来看，应该就是一个体素对应一个位姿，也就是一帧点云
            // 若该体素中存在点云数据，则process_size加1,
            if ((*sig_orig)[i].N != 0)
            {
                process_size++;
            }
    ...
    {
        vector<VOX_FACTOR> sig_trans(win_size);
        int kk = 0;
        int rej_cnt = 0;
        size_t i = 0;
        while(i < plvec_voxels.size())
        {
            ...
            {
                // 主要是将体素内的点云数据转换到当前状态的坐标系下
                sig_trans[j].transform(sig_orig[j], xs[j]);
            }
            ...
            // 若体素内的最小特征值大于阈值，则去除这个体素，物理意义是该体素的特征值较大，可能不是平面
...
// 选择性发布残差以及方向的值
    ...
    void cut_func(int ci)
    {
        ...
        for (uint j = 0; j < pvec_tran.size(); j++)
        {
            int xyz[3] = {0, 0, 0};
            for (uint k = 0; k < 3; k++)
            {
    ...
    void recut()
    {
        ...
        {
            int point_size = 0;
            for (int i = 0; i < win_size; i++) // 统计该窗口内的点云数量
            {
...
public:
    ...
    void damping_iter(vector<IMUST> &x_stats, VOX_HESS &voxhess, double &residual, PLV(6) & hess_vec, size_t &mem_cost)
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            Eigen::SparseMatrix<double> A1_sparse(jac_leng, jac_leng); // 稀疏矩阵
            std::vector<Eigen::Triplet<double>> tripletlist;           // 创建一个 std::vector，用于存储稀疏矩阵的非零元素
            for (int a = 0; a < jac_leng; a++)
            {
                ...
                if (temp_mem > max_mem)
                    max_mem = temp_mem;
                solve_time += ros::Time::now().toSec() - tm;
