...
namespace mypcl
{
    ...
    void transform_pointcloud(pcl::PointCloud<PointType> const &pc_in, pcl::PointCloud<PointType> &pc_out, Eigen::Vector3d t, Eigen::Quaterniond q)
    {
        size_t size = pc_in.points.size();
        pc_out.points.resize(size);
        for(size_t i = 0; i < size; i++)
        {
            Eigen::Vector3d pc_tmp;

            pc_tmp = q * Eigen::Vector3d(pc_in.points[i].x, pc_in.points[i].y, pc_in.points[i].z) + t;
            pc_out.points[i].x = pc_tmp.x();
            pc_out.points[i].y = pc_tmp.y();
            pc_out.points[i].z = pc_tmp.z();
            pc_out.points[i].intensity = pc_in.points[i].intensity;
